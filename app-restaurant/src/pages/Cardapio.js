...
function Cardapio() {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      setLoading(false); // Finaliza o estado de carregamento, com ou sem erro
    }
  };

  // useEffect para buscar os pedidos quando o componente é montado.
  // O array de dependências vazio faz com que ele execute apenas uma vez.
  useEffect(() => {
    fetchOrders();
  }, []);

  // useEffect para recalcular o preço total sempre que a lista de pedidos mudar.
  useEffect(() => {
  ...
  // Lógica de renderização condicional para resolver o erro "no-unused-vars".
  ...
  // Se não houver carregamento nem erro, exibe o conteúdo normal do cardápio.
  return (
    <div className={style.body}>
      <Header />
      <div className={style.pratosContainer}>
